...
void read_subint_params(char *buf, obs_params *g, struct psrfits *p) {
    ...
    get_int(buf,"BLOCSIZE", &g->packets_per_block, 0);
    if (g->packetsize>0)
...
int read_status_params(char *buf, obs_params *g, struct psrfits *p) {
    ...
    
    {
        ...
        get_int(buf,"BLOCSIZE", &p->sub.bytes_per_subint, 0);
	...
	fflush(stdout);
        if (fold) { 
            ...
            get_dbl(buf,"TSUBINT", &p->sub.tsubint, 0.0); 
            p->sub.bytes_per_subint = sizeof(float) * p->hdr.nbin * p->hdr.nchan * p->hdr.npol;
