...
__global__ void reduce(T* idata, T* odata, const unsigned n, const unsigned npts4mean) {
    ...
    if (tid == 0 && blockIdx.x==0 && npts4mean) {
	//printf("%d %f\n", blockIdx.x, odata[blockIdx.x]); 
	odata[blockIdx.x] = mean[0]/(float)npts4mean;
    }

    //if (npts4mean) printf("B> %d %d\n", blockIdx.x, tid);

    if (tid == 0 && !npts4mean) odata[blockIdx.x] = mean[0];
    //if(tid==0) printf("%d %f\n", blockIdx.x, odata[blockIdx.x]);
}
...
template <unsigned int blockSize, typename T>
__global__ void reduce_pass2(T *idata, T *odata, float *mean, float *var, int *count, unsigned int n) {
...
template <unsigned int blockSize, typename T>
__global__ void variance_pass2(T *idata, float *odata, float *mean, float *variance, unsigned int n) {
    __shared__ float var[blockSize];
    const int tid = threadIdx.x;
    int i = blockIdx.x*(blockSize*2) + threadIdx.x;
    const int gridSize = blockSize*2*gridDim.x;
...
__global__ void remove_baseline_scale(float *idata, char *odata, float *mean, float var, const int nbits, size_t n) {
    ...
    const float rms = sqrtf(var/n); 
    const int max = powf(2, nbits) - 1;
    const float tmp = 3 * rms - mean[0];
    const float scale = 6 * rms / powf(2, nbits);

    for (int i=tId; i<n; i+=2*nt) {
	data.x = idata[2*i] + tmp;
	data.y = idata[2*i+1] + tmp;

	idat.x = __float2int_rn(data.x/scale);
	idat.y = __float2int_rn(data.y/scale);

	if (idat.x < 0) idat.x = 0;
	if (idat.x > max) idat.x = max;
	if (idat.y < 0) idat.y = 0;
	if (idat.y > max) idat.y = max;

	*(&odata[2*i]) = idat.x;     // 1st 4 bits (MSBs) are first point
	*(&odata[2*i+1]) = idat.y;		// 2nd 4 bits (LSBs) are second point
	//*(&odata[i]) = idat.x << 4;     // 1st 4 bits (MSBs) are first point
	//*(&odata[i]) = idat.y;		// 2nd 4 bits (LSBs) are second point
