...
databuf *databuf_create(int n_block, size_t block_size, int databuf_id) {
...
databuf *databuf_attach(int databuf_id) {
    ...
    if (shmid==-1) {
        sprintf(strlog,"shmget error (%s) for id #%d key=%d", strerror(errno), databuf_id, DATABUF_KEY + databuf_id - 1);
    ...
    d = shmat(shmid, NULL, 0);
    if (d == (void *)-1) {
        sprintf(strlog,"shmat error for id #%d key=%d", databuf_id, DATABUF_KEY + databuf_id - 1);
        log_error("databuf_attach", strlog);
        return(NULL);
    }
